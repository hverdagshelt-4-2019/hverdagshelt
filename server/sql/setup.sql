...
DROP TABLE IF EXISTS private_person_CROSS_commune;
...
CREATE TABLE private_person_CROSS_commune(
  person_id INT,
  commune_id INT,

  PRIMARY KEY (person_id, commune_id),
  FOREIGN KEY (person_id) REFERENCES private_person(id) ON DELETE CASCADE,
...
CREATE TABLE ticket(
  ...
  FOREIGN KEY (submitter_id) REFERENCES private_person(id),
